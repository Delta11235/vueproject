<template>
    <div v-if="items">
        ...
        <option v-for="rate in items[0].rates" :key="rate.no">{{ rate.code }}</option>
        </select>
        ...
        <div v-if="api">
            ...
            <tr>
                <td>data</td>
                <td v-for="rate in api.rates" :key="rate.no">{{ rate.effectiveDate }}</td>
            </tr>
            <tr>
                <td>id</td>
...
<script>
    export default {
        ...
        updated(){
        fetch('https://api.nbp.pl/api/exchangerates/rates/A/'+ this.code +'/last/'+this.iloscDni +'/?format=json')
            .then(res => res.json())
